///#include <vulkan/vulkan.h>//this is off-screen rendering, so replace it as:
#define GLFW_INCLUDE_VULKAN
#include <GLFW/glfw3.h>//will automatically loadthe Vulkan header
...
#include <iostream>
#include <stdexcept> //iostream and stdexcept - report and propagate errors
#include <cstdlib> //provide ECIT_SUCCESS & EXIT_FAILURE macros
#include <cstring>
#include <vector>
#include <map>
#include <optional>
...
private:
	...
	void createDescriptorSetLayout() {
		...
		uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; //just reference vertex shader
	...
	void createRenderPass() {
		...
		colorAttachment.samples = VK_SAMPLE_COUNT_1_BIT; //related to multisampling
		colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
		colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
		colorAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		colorAttachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		colorAttachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		colorAttachment.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;//images to obe presented in the swap chain

		VkAttachmentReference colorAttachmentRef{};
		colorAttachmentRef.attachment = 0;//one single VkAttachementDescripstion so it's 0 as index (the first one)
	...
	void createGraphicsPipeline() {
		...
		auto attribureDescriptions = Vertex::getAttributeDescriptions();

		vertexInputInfo.vertexBindingDescriptionCount = 1;
		vertexInputInfo.pVertexBindingDescriptions = &bindingDescription;
		vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(attribureDescriptions.size());
		vertexInputInfo.pVertexAttributeDescriptions = attribureDescriptions.data();
		...
		colorBlending.blendConstants[3] = 0.0f;//can be used as blend factors
		...
		pipelineInfo.pStages = shaderStages; //vert and frag
		...
		pipelineInfo.subpass = 0;//it's index
	...
	void createInstance() {
		//check if the validation layer is enabled but not supported
	...
	bool isDeviceSuitable(VkPhysicalDevice device) {
		...
		//check if the swap chain support is enough
		...
		//e.g. only usable for dedicated graphics cards that support geometry shaders
	...
	bool checkValidationLayerSupport() {
		...
		//check if all validationLayers exsit in the availableLayers list
	...
	//message call back - not completed
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
		VkDebugUtilsMessageSeverityFlagBitsEXT mesnssageSeverity,
